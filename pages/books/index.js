...
const GENRES = ['全部', '小说', '传记', '文学', '历史', '哲学', '社科', '心理', '经济', '政治', '其他'];
...
Page({
  ...
  onAddTitleInput(e) {
    this.setData({
      'addForm.title': e.detail.value
    });
  },

  onAddAuthorInput(e) {
    this.setData({
      'addForm.author': e.detail.value
    });
  },

  onAddNotesInput(e) {
    this.setData({
      'addForm.notes': e.detail.value
    });
  },

  onAddGenreTap(e) {
    const genre = e.currentTarget.dataset.genre;
    if (genre) this.setData({
      'addForm.genre': genre
    });
  },
